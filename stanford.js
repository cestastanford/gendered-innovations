...
queue()
    .defer(d3.csv, "./d/network_stanford.csv")
...
function ready(error, csv_data, world, locations) {
  ...
  var stanford_svg = d3.select("#stanford_network").append("svg")
      .attr("width", width)
      .attr("height", height);

  stanford_svg.append("rect")
    .attr("width", width)
    .attr("height", height);

  stanford_svg.append("text")
  ...
  // add the edges
  var path = stanford_svg.append("g").selectAll("path")
    .data(force.links())
  .enter().append("path")
    .attr("class", "link");

  // define the nodes
  var node = stanford_svg.selectAll(".node")
    .data(force.nodes())
  .enter().append("g")
    .attr("class", "node")
