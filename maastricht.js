...
queue()
    .defer(d3.csv, "./d/network_maastricht.csv")
...
function ready(error, csv_data, world, locations) {
  ...
  var maastricht_svg = d3.select("#maastricht_network").append("svg")
      .attr("width", width)
      .attr("height", height);

  maastricht_svg.append("rect")
    .attr("width", width)
    .attr("height", height);

  maastricht_svg.append("text")
  ...
  // add the edges
  var path = maastricht_svg.append("g").selectAll("path")
    .data(force.links())
  .enter().append("path")
    .attr("class", "link");

  // define the nodes
  var node = maastricht_svg.selectAll(".node")
    .data(force.nodes())
  .enter().append("g")
    .attr("class", "node")
