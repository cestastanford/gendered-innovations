...
queue()
    .defer(d3.csv, "./d/network_madrid.csv")
...
function ready(error, csv_data, world, locations) {
  ...
  var madrid_svg = d3.select("#madrid_network").append("svg")
      .attr("width", width)
      .attr("height", height);

  madrid_svg.append("rect")
    .attr("width", width)
    .attr("height", height);

  madrid_svg.append("text")
  ...
  // add the edges
  var path = madrid_svg.append("g").selectAll("path")
    .data(force.links())
  .enter().append("path")
    .attr("class", "link");

  // define the nodes
  var node = madrid_svg.selectAll(".node")
    .data(force.nodes())
  .enter().append("g")
    .attr("class", "node")
