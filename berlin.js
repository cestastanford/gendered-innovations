...
queue()
    .defer(d3.csv, "./d/berlin_network.csv")
...
function ready(error, csv_data, world, locations) {
  ...
  var berlin_map_projection = d3.geo.eckert3()
      .translate([width / 2, height / 2])
      .rotate([50, 0])
      .center([65, 52])
      .scale(550);

  var berlin_map_path = d3.geo.path()
      .projection(berlin_map_projection);

  var berlin_svg = d3.select("#berlin_network").append("svg") // #berlin_network
      .attr("width", width)
      .attr("height", height);

  berlin_svg.append("rect")
    .attr("width", width)
    .attr("height", height);

  var g = berlin_svg.append("g");

  g.selectAll("path")
      .data(topojson.feature(world, world.objects.countries).features)
    .enter().append("path")
      .attr("d", berlin_map_path)
      .attr("class", "feature");
  ...
  // add the edges
  var path = berlin_svg.append("g").selectAll("path")
    .data(force.links())
  .enter().append("path")
    .attr("class", "link");

  // define the nodes
  var node = berlin_svg.selectAll(".node")
    .data(force.nodes())
  .enter().append("g")
    .attr("class", "node")
  ...
  node.append("text")
    ...
    for (n in nodes) {
      if(nodes[n].long) {
        var projectedNode = berlin_map_projection([nodes[n].long,nodes[n].lat])
