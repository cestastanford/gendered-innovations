...
queue()
    .defer(d3.csv, "./d/network_paris.csv")
...
function ready(error, csv_data, world, locations) {
  ...
  var paris_svg = d3.select("#paris_network").append("svg")
      .attr("width", width)
      .attr("height", height);

  paris_svg.append("rect")
    .attr("width", width)
    .attr("height", height);

  paris_svg.append("text")
  ...
  // add the edges
  var path = paris_svg.append("g").selectAll("path")
    .data(force.links())
  .enter().append("path")
    .attr("class", "link");

  // define the nodes
  var node = paris_svg.selectAll(".node")
    .data(force.nodes())
  .enter().append("g")
    .attr("class", "node")
