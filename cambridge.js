...
queue()
    .defer(d3.csv, "./d/cambridge_network.csv")
...
function ready(error, csv_data, world, locations) {
  ...
  var cambridge_svg = d3.select("#cambridge_network").append("svg")
      .attr("width", width)
      .attr("height", height);

  cambridge_svg.append("rect")
    .attr("width", width)
    .attr("height", height);

  cambridge_svg.append("text")
  ...
  // add the edges
  var path = cambridge_svg.append("g").selectAll("path")
    .data(force.links())
  .enter().append("path")
    .attr("class", "link");

  // define the nodes
  var node = cambridge_svg.selectAll(".node")
    .data(force.nodes())
  .enter().append("g")
    .attr("class", "node")
